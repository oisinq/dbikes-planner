...
def predict_station(station, minutes, request_type):
    return requests.get(f"{flask.request.host_url}/predict/{request_type}?minutes={minutes}&station={station['address']}")
...
def get_cycle_streets_url(start, end):
    return f"https://www.cyclestreets.net/api/journey.json?key=b81a31ea96441895&itinerarypoints={start['lng']}," \
           f"{start['lat']}|{end['lng']},{end['lat']}&plan=quietest"
...
def best_station(location, station_list, station_tree, request_type, minutes):
    ...
    for index, station_index in enumerate(closest_start_stations[1]):
        ...
        response = predict_station(station_list.iloc[station_index], minutes, 'bikes')
        json = response.json()
        availability = json['prediction']

        if availability is 'high':
            best_station = station_list.iloc[station_index]
...
def generate_route():
    ...
    start_station = best_station(start_location, station_list, station_tree, 'bikes', minutes)
    end_station = best_station(end_location, station_list, station_tree, 'bikestands', minutes)
    ...
    return jsonify({"start_walking_route": start_walking_route_json.json(), "cycle_route": cycle_route_json.json(),
                    "end_walking_route": end_walking_route_json.json()})
